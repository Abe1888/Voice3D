...
export class RagService {
    ...
    async retrieveContext(query) {
        ...
        const relevant = internalKnowledge.filter(k => query.toLowerCase().split(' ').some(word => k.toLowerCase().includes(word)));
        if (relevant.length === 0)
            return "No specific Translink docs found for this query.";
